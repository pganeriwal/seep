if (thisPeerName && thisPeerId && otherPeerId) {
    ...
    var sendMessageToPeers = function (message, peerIds) {
        const ret = {
            sentToAll: false,
            sentToAny: false
        };
     ...
     * @returns 
     */
    var connectToOtherPeer = function (peerId) {
        const ret = {
...
var app = new Vue({
    ...
    methods: {
        connectToOtherPeer: function () {
            const ret = connectToOtherPeer(this.otherPeerId);
            if (ret && ret.error) {
                this.message = ret.error;
            }
        },
        sendMessageToPeers: function () {
            const ret = sendMessageToPeers(this.messageForPeer);
        }
    }
});